...
function ExperienceItem({ item, index }: { item: Experience; index: number }) {
  ...
  return (
    <Spotlight className="w-full rounded-2xl sm:rounded-3xl">
      <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.55, delay: index * 0.08, ease: [0.16, 1, 0.3, 1] }} className="group relative w-full rounded-2xl border border-white/[0.06] bg-black/40 p-6 backdrop-blur-xl transition-all duration-300 hover:border-white/[0.12] hover:bg-black/55 sm:rounded-3xl sm:p-8">
    ...
    <section id="experience" className="relative scroll-mt-24 px-6 py-28 sm:py-36">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="flex w-full flex-col gap-6 sm:gap-8">
          {experience.map((item, index) => (<ExperienceItem key={`${item.company}-${index}`} item={item} index={index} />))}
        </div>
      </div>
    </section>
  );
