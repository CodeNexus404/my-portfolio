...
export default function Skills() {
  ...
  // Flatten every group's items into one pool, then split across the two
  // marquee streams (forward + reverse) so the sliding rows stay populated
  // no matter how the owner edits the groups from the dashboard.
  const allItems = groups.flatMap((g) => g.items.filter(Boolean));
  const streamOne = allItems.length ? allItems : levelOne;
  const streamTwo = allItems.length ? allItems : levelTwo;

  const levels = [
    { items: buildTrack(streamOne), reverse: false },
    { items: buildTrack(streamTwo), reverse: true },
  ];
  ...
  return (
    <section id="skills" className="relative scroll-mt-24 px-6 py-28 sm:py-36">
      <div className="mx-auto max-w-6xl">
        ...
        >
          {allItems.length} tools &middot; 2 streams &middot; pause on hover
        </motion.p>
        {note ? (
          <motion.p
            initial={{ opacity: 0 }}
